...
def test_order_summary_with_service(page: Page):
    """
    This test verifies that booked services are displayed in the order summary.
    """
    ...
    # This assumes the service name is unique enough to be found.
    # A more robust selector might be needed if there are multiple services.
    expect(page.get_by_text("Home Barista Training")).to_be_visible()

    # 5. Screenshot: Capture the final result for visual verification.
    page.screenshot(path="jules-scratch/verification/order_summary_verification.png")