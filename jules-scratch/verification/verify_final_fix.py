...
def test_order_summary_fix(page: Page):
    """
    This test verifies that the booked services are correctly displayed in the order summary
    after fixing the filtering logic.
    """
    ...
    # Using a more specific locator to find the service name within the summary.
    # This assumes the service name is "Home Barista Training" as seen in previous attempts.
    # If the service name changes, this selector might need to be updated.
    expect(page.locator("div.space-y-4.border-t-2").get_by_text("Home Barista Training")).to_be_visible()

    # 5. Screenshot: Capture the final result for visual verification.
    page.screenshot(path="jules-scratch/verification/final_fix_verification.png")