...
export class BoroughCampaignsService {
  ...
  async participate(campaignId: string, businessId: string): Promise<BoroughParticipation> {
    const campaign = await this.campaignRepository.findOne({ where: { id: campaignId } });
    if (!campaign) {
      throw new NotFoundException(`Borough campaign with ID ${campaignId} not found`);
    }
