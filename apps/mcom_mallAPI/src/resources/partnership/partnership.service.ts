...
export class PartnershipService {
  ...
  async searchPartnerItems(
    ...
    const products = await this.productRepository
      ...
      .andWhere(
        new Brackets((qb) => {
          qb.where('p.title ILIKE :searchTerm', { searchTerm }).orWhere(
    ...
    const services = await this.serviceRepository
      ...
      .andWhere(
        new Brackets((qb) => {
          qb.where('s.name ILIKE :searchTerm', { searchTerm }).orWhere(
            's.description ILIKE :searchTerm',
            { searchTerm },
          );
        }),
      )
  ...
  async createCompositePartnershipRequest(
    ...
    if (dto.plusProductId) {
      ...
      const s = await this.serviceRepository.findOne({
        where: { id: dto.plusServiceId },
        relations: ['business', 'business.user'],
      });
    ...
    const userPartnership = await this.userPartnershipRepository.findOne({
      where: [
        {
          user1: { id: currentUser.id },
          user2: { id: targetOwnerId },
          isActive: true,
        },
        {
          user1: { id: targetOwnerId },
    ...
    if (!userPartnership) {
        ...
        where: [
          {
            ...
            receiver: { id: targetOwnerId },
            status: PartnershipStatus.PENDING,
          },
          {
            sender: { id: targetOwnerId },
      ...
      if (!userRequest) {
        ...
        await this.emailService.sendPartnershipRequestEmail(
          targetUser,
          currentUser,
        );
      }
    ...
    const savedItemRequest =
      await this.itemPartnershipRequestRepository.save(itemRequest);
    ...
    await this.emailService.sendPartnershipRequestEmail(
      ...
      {
        baseItemName:
          (fullBaseItem as any)?.title ||
          (fullBaseItem as any)?.name ||
          'Base Item',
        plusItemName:
          (fullPlusItem as any)?.title ||
          (fullPlusItem as any)?.name ||
          'Plus Item',
      },
    );
  ...
  async createUserPartnershipRequest(
    dto: CreateUserPartnershipRequestDto,
    sender: User,
    ...
    const savedRequest =
      await this.userPartnershipRequestRepository.save(request);
    await this.emailService.sendPartnershipRequestEmail(receiver, sender);

    return savedRequest;
  }

  async respondToUserPartnershipRequest(
    id: string,
    dto: RespondToUserPartnershipRequestDto,
    user: User,
  ): Promise<UserPartnershipRequest> {
    ...
    if (dto.status === PartnershipStatus.ACCEPTED) {
      ...
      const savedPartnership =
        await this.userPartnershipRepository.save(partnership);

      // Link any orphan item requests
      const orphanRequests = await this.itemPartnershipRequestRepository.find({
        where: [
          {
            proposer: { id: request.sender.id },
            receiver: { id: request.receiver.id },
            status: PartnershipStatus.PENDING,
            partnership: null,
          },
          {
            proposer: { id: request.receiver.id },
            receiver: { id: request.sender.id },
            status: PartnershipStatus.PENDING,
            partnership: null,
          },
        ],
  ...
  async getMyPartners(user: User): Promise<any[]> {
    const partnerships = await this.userPartnershipRepository.find({
      ...
      relations: [
        'user1',
        'user1.businesses',
        'user1.businesses.location',
        'user2',
        'user2.businesses',
        'user2.businesses.location',
        'request',
      ],
    });
    ...
    return partnerships.map((p) => {
      ...
      const postcodes =
        partner.businesses?.map((b) => b.location?.postcode).filter(Boolean) ||
        [];
      return {
        partnershipId: p.id,
  ...
  async createItemPartnershipRequest(
    dto: CreateItemPartnershipRequestDto,
    proposer: User,
    ...
    if (
      partnership.user1.id !== proposer.id &&
      partnership.user2.id !== proposer.id
    ) {
    ...
    const partnerId =
      partnership.user1.id === proposer.id
        ? partnership.user2.id
        : partnership.user1.id;
    ...
    if (dto.baseProductId) {
      ...
      if (!p || p.business.user.id !== proposer.id)
        throw new BadRequestException('You do not own the base product');
    }
    if (dto.baseServiceId) {
      ...
      if (!s || s.business.user.id !== proposer.id)
        throw new BadRequestException('You do not own the base service');
    }
    if (dto.plusProductId) {
      const p = await this.productRepository.findOne({
        where: { id: dto.plusProductId },
      ...
      if (!p || p.business.user.id !== partnerId)
        throw new BadRequestException('Partner does not own the plus product');
    }
    if (dto.plusServiceId) {
      ...
      if (!s || s.business.user.id !== partnerId)
        throw new BadRequestException('Partner does not own the plus service');
    }
    ...
    const request = this.itemPartnershipRequestRepository.create({
      ...
      proposer,
      receiver: partner,
      baseProduct: dto.baseProductId
        ? ({ id: dto.baseProductId } as Product)
        : null,
      baseService: dto.baseServiceId
        ? ({ id: dto.baseServiceId } as Service)
        : null,
      plusProduct: dto.plusProductId
        ? ({ id: dto.plusProductId } as Product)
        : null,
      plusService: dto.plusServiceId
        ? ({ id: dto.plusServiceId } as Service)
        : null,
      status: PartnershipStatus.PENDING,
      sentAt: new Date(),
    ...
    const savedRequest =
      await this.itemPartnershipRequestRepository.save(request);
  ...
  async respondToItemPartnershipRequest(
    id: string,
    dto: RespondToUserPartnershipRequestDto,
    user: User,
  ): Promise<ItemPartnershipRequest> {
    ...
    if (request.partnership) {
      const partnerId =
        request.partnership.user1.id === request.proposer.id
          ? request.partnership.user2.id
          : request.partnership.user1.id;
    ...
    if (dto.status === PartnershipStatus.ACCEPTED && !request.partnership) {
      throw new BadRequestException(
        'Cannot accept item request until user partnership is active',
      );
    }
  ...
  async getPartnerItems(partnershipId: string, user: User): Promise<any> {
    ...
    if (partnership.user1.id !== user.id && partnership.user2.id !== user.id)
      throw new UnauthorizedException();

    const partnerId =
      partnership.user1.id === user.id
        ? partnership.user2.id
        : partnership.user1.id;
