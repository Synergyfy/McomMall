...
export class TerminalCashbackService {
  ...
  async getClaimById(id: string): Promise<TerminalCashbackClaim> {
    const claim = await this.claimRepository.findOne({ where: { id }, relations: ['user'] });
  ...
  async updateClaimStatus(id: string, status: TerminalCashbackStatus): Promise<TerminalCashbackClaim> {
    ...
    if (status === TerminalCashbackStatus.APPROVED || status === TerminalCashbackStatus.AUTO_APPROVED) {
