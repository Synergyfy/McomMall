...
import { TransactionQueryDto, PaginatedTransactionsDto, TransactionStatsDto, AdminTransactionDto } from '../dto/transactions.dto';
...
export class AdminTransactionsService {
  ...
  async getStats(): Promise<TransactionStatsDto> {
    const [orderSum, bookingSum, membershipSum, orderPending, bookingPending] = await Promise.all([
      this.ordersRepository.createQueryBuilder('o').select('SUM(o.total)', 'sum').getRawOne(),
      this.bookingsRepository.createQueryBuilder('b').leftJoin('b.payment', 'p').select('SUM(p.amount)', 'sum').getRawOne(),
      this.membershipPaymentsRepository.createQueryBuilder('m').select('SUM(m.amount)', 'sum').getRawOne(),
  ...
  async findAll(query: TransactionQueryDto): Promise<PaginatedTransactionsDto> {
    ...
    const [orders, bookings, memberships] = await Promise.all([
       this.ordersRepository.find({
           relations: ['user', 'payment', 'items', 'items.product', 'items.product.business'],
    ...
    const allTransactions: AdminTransactionDto[] = [
      ...orders.map(o => ({
        ...
        payeeName: o.items?.[0]?.product?.business?.businessName || 'Multiple/Unknown',
