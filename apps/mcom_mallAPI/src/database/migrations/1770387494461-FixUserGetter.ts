...
export class FixUserGetter1770387494461 implements MigrationInterface {
  name = 'FixUserGetter1770387494461';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Safely migrate User names
    await queryRunner.query(
      `DO $$ BEGIN ALTER TABLE "users" ADD "firstName" character varying; EXCEPTION WHEN duplicate_column THEN END $$`,
    );
    await queryRunner.query(
      `DO $$ BEGIN ALTER TABLE "users" ADD "lastName" character varying; EXCEPTION WHEN duplicate_column THEN END $$`,
    );

    // Split name into firstName and lastName (Postgres specific)
    await queryRunner.query(`UPDATE "users" SET 
            "firstName" = split_part("name", ' ', 1),
            "lastName" = substr("name", length(split_part("name", ' ', 1)) + 2)
        `);

    // Default values for empty ones to avoid NOT NULL violation
    await queryRunner.query(
      `UPDATE "users" SET "firstName" = 'User' WHERE "firstName" IS NULL OR "firstName" = ''`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DO $$ BEGIN ALTER TABLE "users" ALTER COLUMN "firstName" SET NOT NULL; EXCEPTION WHEN undefined_column THEN END $$`,
    );
    await queryRunner.query(
      `DO $$ BEGIN ALTER TABLE "users" ALTER COLUMN "lastName" SET NOT NULL; EXCEPTION WHEN undefined_column THEN END $$`,
    );
    await queryRunner.query(
      `DO $$ BEGIN ALTER TABLE "users" DROP COLUMN "name"; EXCEPTION WHEN undefined_column THEN END $$`,
    );
    ...
    await queryRunner.query(
      `DO $$ BEGIN ALTER TABLE "otps" ADD "email" character varying; EXCEPTION WHEN duplicate_column THEN END $$`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE "otps" DROP COLUMN "email"`);
