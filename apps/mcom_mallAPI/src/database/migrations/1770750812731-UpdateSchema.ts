...
export class UpdateSchema1770750812731 implements MigrationInterface {
  name = 'UpdateSchema1770750812731';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "terminal_cashback_claims" RENAME COLUMN "businessId" TO "ownerId"`,
    );
    await queryRunner.query(
    ...
    // Fix terminal_configs migration to handle existing data
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" DROP CONSTRAINT "PK_2be7ff8664fe267c9dbc78ab38c"`,
    );
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" ADD "userId" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" ADD "userName" character varying`,
    );

    // Copy data if columns exist
    await queryRunner.query(
      `UPDATE "terminal_configs" SET "userId" = "businessId", "userName" = "businessName"`,
    );

    // Ensure we don't have nulls before making NOT NULL
    await queryRunner.query(
      `UPDATE "terminal_configs" SET "userId" = 'system' WHERE "userId" IS NULL`,
    );
    await queryRunner.query(
      `UPDATE "terminal_configs" SET "userName" = 'System' WHERE "userName" IS NULL`,
    );

    await queryRunner.query(
      `ALTER TABLE "terminal_configs" ALTER COLUMN "userId" SET NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" ALTER COLUMN "userName" SET NOT NULL`,
    );

    await queryRunner.query(
      `ALTER TABLE "terminal_configs" DROP COLUMN "businessId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" DROP COLUMN "businessName"`,
    );

    await queryRunner.query(
      `ALTER TABLE "terminal_configs" ADD CONSTRAINT "PK_9ae67ef95353d2c7482fd1fd22a" PRIMARY KEY ("id", "userId")`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" DROP COLUMN "userName"`,
    );
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" DROP CONSTRAINT "PK_9ae67ef95353d2c7482fd1fd22a"`,
    );
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" DROP COLUMN "userId"`,
    );

    await queryRunner.query(
      `ALTER TABLE "terminal_configs" ADD "businessName" character varying NOT NULL DEFAULT 'System'`,
    );
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" ADD "businessId" character varying NOT NULL DEFAULT 'system'`,
    );
    await queryRunner.query(
      `ALTER TABLE "terminal_configs" ADD CONSTRAINT "PK_2be7ff8664fe267c9dbc78ab38c" PRIMARY KEY ("id", "businessId")`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "terminal_cashback_claims" RENAME COLUMN "ownerId" TO "businessId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "service_provider_profiles" ADD CONSTRAINT "FK_f143f396ea55404f8ed5a5421a3" FOREIGN KEY ("businessId") REFERENCES "businesses"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
