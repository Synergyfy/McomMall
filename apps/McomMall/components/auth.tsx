...
const Auth = ({ redirect }: { redirect: string | null }) => {
  ...
  const handleSubmitRegistration = async () => {
    ...
    try {
      ...
      } else if (selectedRole === UserRole.OWNER || selectedRole === UserRole.ADMIN) {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (response.role === UserRole.OWNER || response.role === UserRole.ADMIN) router.push('/dashboard');
      else router.push('/');
    } catch (error: unknown) {
      const err = error as ErrorResponse;
