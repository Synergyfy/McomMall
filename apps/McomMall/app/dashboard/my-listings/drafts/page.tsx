...
const ListingCard: React.FC<{
    ...
    return (
        ...
        >
            <Card className="w-full overflow-hidden border-orange-100 bg-orange-50/10">
                <div className="flex flex-col md:flex-row items-center gap-6 p-5">
                    <div className="relative h-24 w-24 md:h-28 md:w-28 shrink-0 overflow-hidden rounded-xl border border-slate-100 bg-slate-50">
                        {imgSrc ? (
                            <img src={imgSrc} alt={listing.businessName} onError={() => setImgSrc(null)} className="absolute inset-0 w-full h-full object-cover" />
                        ) : (
                    ...
                    <div className="flex-grow text-center md:text-left space-y-2">
                        <div className="flex flex-col md:flex-row md:items-center gap-2">
                            ...
                            <Badge
                                variant="outline"
                                className="w-fit mx-auto md:mx-0 font-bold uppercase tracking-wider text-[10px] border-none bg-orange-50 text-orange-600"
                            >
                                Draft
                            </Badge>
                        </div>
                    ...
                    <div className="flex flex-row md:flex-col items-center gap-3 w-full md:w-auto">
                        <Button
                            variant="outline"
                            className="flex-1 md:w-full h-11 px-4 gap-2 border-orange-200 text-orange-600 hover:bg-orange-50"
                            onClick={e => {
                                e.stopPropagation();
                                router.push(`/dashboard/edit-listing/${listing.id}`);
                            }}
...
export default function DraftListingsPage() {
    ...
    const {
        ...
        isError,
        error,
    } = useGetUserListings(page, limit, 'draft');
    ...
    const listings = useMemo(() => {
        return (paginatedData?.data || []).filter(l => l.status?.toLowerCase() === 'draft');
    }, [paginatedData]);
        ...
        <div className="min-h-screen bg-slate-50/50 p-6 md:p-10">
            <div className="mx-auto max-w-5xl space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div className="space-y-1">
                        <h1 className="text-4xl font-black tracking-tight text-slate-900">Draft <span className="text-orange-600">Listings</span></h1>
                        <p className="text-slate-500 font-medium italic">Resume working on your unpublished listing storefronts</p>
                    </div>
                    <Button
                        variant="primary"
                ...
                <div className="space-y-4">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-24 gap-4">
                            ...
                            <p className="text-slate-400 font-bold animate-pulse">Syncing with drafts...</p>
                        </div>
                    ) : isError ? (
                        <div className="rounded-3xl border border-red-100 bg-red-50/50 p-12 text-center">
                            ...
                            <p className="text-red-600/70">{error?.message || "Failed to load draft listings"}</p>
                        ...
                        <div className="flex flex-col items-center justify-center py-24 text-center rounded-3xl border-2 border-dashed border-slate-200 bg-white space-y-4">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-slate-900">No draft listings</h3>
                                <p className="text-slate-500">You don&apos;t have any listings in progress at the moment.</p>
                            </div>
                            <Button onClick={() => router.push('/dashboard/add-listing')} variant="outline">Create a new listing</Button>
                        </div>
                    )}
                </div>
            ...
            >
                <AlertDialogContent className="rounded-2xl border-none shadow-2xl">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="text-2xl font-black">Discard Draft?</AlertDialogTitle>
                        <AlertDialogDescription className="text-slate-500 font-medium">
                            This will permanently delete your draft listing. This action is irreversible.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="gap-2">
                        <AlertDialogCancel className="rounded-xl font-bold">Keep Draft</AlertDialogCancel>
                        <AlertDialogAction
