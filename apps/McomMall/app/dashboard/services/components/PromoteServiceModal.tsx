...
export const PromoteServiceModal: React.FC<PromoteServiceModalProps> = ({
  service,
  open,
  onClose,
}) => {
  ...
  // Sync with current service values when modal opens
  useEffect(() => {
    if (service) {
      setIsFeatured(service.isFeatured ?? false);
      setIsRotatorEligible(service.isRotatorEligible ?? false);
      setIsPromotionEligible(service.isPromotionEligible ?? false);
    }
  }, [service]);
  ...
  const handleApply = async () => {
    try {
      ...
      toast.success('Service promotion settings updated!');
      onClose();
    } catch (err: any) {
      toast.error(err.response?.data?.message || err.message || 'Failed to update. Please try again.');
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-sm sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-lg font-bold">Promote Service</DialogTitle>
          <DialogDescription className="text-sm text-gray-500">
            Control where <span className="font-semibold text-gray-700">{service.name}</span> appears across the platform.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
                  Pins this service prominently at the top of your public storefront page.
                </p>
              </div>
            </div>
            <Switch
          ...
          <div className="flex items-start justify-between gap-4 p-4 rounded-xl border border-gray-100 bg-blue-50/40 hover:bg-blue-50 transition-colors">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
                  Opts this service into the Borough carousel and Local Mall discovery feed.
                </p>
              </div>
            </div>
            <Switch
          ...
          <div className="flex items-start justify-between gap-4 p-4 rounded-xl border border-gray-100 bg-orange-50/40 hover:bg-orange-50 transition-colors">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
                  Allows this service to be picked up by platform-wide deals and marketing campaigns.
                </p>
              </div>
            </div>
            <Switch
