...
import { Search, LayoutGrid, ListIcon, Loader2, Ticket } from 'lucide-react';
...
import { useGetPublicVouchers } from '@/service/marketplace/discovery';
...
export default function VouchersPage() {
    ...
    const { data: vouchersData, isLoading } = useGetPublicVouchers({
        page: currentPage,
    ...
    // Transform vouchers to display items
    const displayItems = useMemo(() => {
        if (!vouchersData?.data) return [];

        return vouchersData.data.map((item: MarketItem) => {
            const title = item.title || item.name || 'Untitled Voucher';
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="mb-6">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h1 className="text-3xl md:text-4xl font-bold text-gray-900">All Vouchers</h1>
                            <p className="text-gray-600 mt-1">Discover exclusive voucher deals and savings</p>
                        </div>
                    </div>
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 sticky top-28 z-30">
                    ...
                    <div className="mt-4 flex items-center justify-between text-sm text-gray-500 border-t pt-4">
                        <p>Showing <span className="font-semibold text-gray-900">{displayItems.length}</span> vouchers</p>
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
                        ...
                        <h3 className="text-lg font-bold text-gray-900 mb-1">No vouchers found</h3>
                        <p className="text-gray-500">
                            {searchQuery ? "Try adjusting your search." : "No vouchers available right now."}
                        </p>
                        {searchQuery && (
                            <Button
                                variant="link"
                ...
                {/* Pagination */}
                {vouchersData?.meta && vouchersData.meta.totalPages > 1 && (
                    <div className="mt-12">
                        <Pagination
                            currentPage={vouchersData.meta.currentPage}
