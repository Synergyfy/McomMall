...
import { Search, LayoutGrid, ListIcon, Loader2, Briefcase } from 'lucide-react';
...
import { useGetPublicServices } from '@/service/marketplace/discovery';
...
export default function ServicesPage() {
    ...
    // Fetch Categories (Using same global categories for now)
    ...
    const { data: servicesData, isLoading } = useGetPublicServices({
        page: currentPage,
    ...
    const displayItems = useMemo(() => {
        ...
        return servicesData.data.map((item: MarketItem) => {
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="mb-6">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-600 mt-1">Discover professional services for your needs</p>
                        </div>
                    </div>
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 sticky top-28 z-30">
                    ...
                    <div className="mt-4 flex items-center justify-between text-sm text-gray-500 border-t pt-4">
                        <p>Showing <span className="font-semibold text-gray-900">{displayItems.length}</span> services</p>
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    >
                        {displayItems.map((item) => (
                            <ServiceCard key={item.id} service={item} viewMode={viewMode} />
                        ))}
                    </motion.div>
                ) : (
                    <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
                        ...
                        <p className="text-gray-500">
                            {searchQuery ? "Try adjusting your search." : "No services available right now."}
                        </p>
                        {searchQuery && (
                            <Button
                                variant="link"
                ...
                {servicesData?.meta && servicesData.meta.totalPages > 1 && (
                    <div className="mt-12">
                        <Pagination
                            currentPage={servicesData.meta.currentPage}
                            totalPages={servicesData.meta.totalPages}
