...
import { Search, LayoutGrid, ListIcon, Loader2, Gift } from 'lucide-react';
...
export default function GiftCardsPage() {
    ...
    const displayItems = useMemo(() => {
        if (!giftCardsData?.data) return [];

        return giftCardsData.data.map((item: MarketItem) => {
            const title = item.title || item.name || 'Untitled Gift Card';
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 sticky top-28 z-30">
                    ...
                    <div className="mt-4 flex items-center justify-between text-sm text-gray-500 border-t pt-4">
                        <p>Showing <span className="font-semibold text-gray-900">{displayItems.length}</span> gift cards</p>
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    >
                        {displayItems.map((item) => (
                            <GiftCardCard key={item.id} giftCard={item} viewMode={viewMode} />
                        ))}
                    </motion.div>
                ) : (
                    <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
                        ...
                        <h3 className="text-lg font-bold text-gray-900 mb-1">No gift cards found</h3>
                        <p className="text-gray-500">
                            {searchQuery ? "Try adjusting your search." : "No gift cards available right now."}
                        </p>
                        {searchQuery && (
                            <Button
                                variant="link"
                ...
                {/* Pagination */}
                {giftCardsData?.meta && giftCardsData.meta.totalPages > 1 && (
                    <div className="mt-12">
                        <Pagination
