...
import { useGetSoldVouchers } from '@/service/hooks/useVoucherService';
import { Voucher } from '@/service/vouchers/types';
import { CURRENCY } from '@/lib/utils';

type SoldVoucherRowProps = {
  voucher: Voucher;
};
...
const SoldVoucherRow: React.FC<SoldVoucherRowProps> = ({ voucher }) => {
  const rowVariants = {
...
export default function SoldVouchersPage() {
  const { soldVouchers, isLoading, isError } = useGetSoldVouchers();
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <main className="container mx-auto px-4 py-8">
        <header className="mb-8">
          <h1 className="text-4xl font-bold text-slate-800">Sold Vouchers</h1>
                ...
                {isError && (
                  <tr>
                    <td colSpan={7} className="py-4 text-center text-red-500">
                      Error loading sold vouchers.
                    </td>
                  </tr>
                )}
                {soldVouchers?.map(voucher => (
                  <SoldVoucherRow key={voucher.id} voucher={voucher} />
                ))}
              </motion.tbody>
            </table>
          </div>
