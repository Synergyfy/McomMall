    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center text-sm text-muted-foreground">
            ...
            Group Wallet Balance: <span className="font-semibold text-primary ml-1">£{group.wallet.balance.toFixed(2)}</span>
        </div>
        <div>
          <h4 className="font-semibold mb-2">Members</h4>
          <div className="flex flex-wrap gap-2">
            {group.members.map(member => (
              <div key={member.id} className="flex items-center space-x-2 bg-secondary p-2 rounded-md">
        ...
        <p className="text-xs text-muted-foreground pt-4">
            Recruitment deadline: {new Date(group.recruitmentDeadline).toLocaleDateString()}
        </p>
      </CardContent>
    </Card>
  );
...
const GroupsClient = () => {
  ...
  const onJoinSubmit = (values: z.infer<typeof joinGroupSchema>) => {
    joinGroup.mutate({ groupId: values.groupId }, {
      onSuccess: () => {
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 space-y-8">
      ...
      <Card>
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onJoinSubmit)} className="flex items-start gap-4">
              <FormField
              ...
              />
              <Button type="submit" disabled={joinGroup.isPending} className="mt-auto">
