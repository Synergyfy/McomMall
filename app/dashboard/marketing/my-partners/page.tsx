...
const EmptyState = ({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) => (
    <motion.div
        ...
        className="text-center p-12 border-2 border-dashed rounded-2xl bg-slate-50/50"
    >
...
const SentRequestCard = ({ request }: { request: PartnershipRequest }) => {
  ...
  return (
    <motion.div whileHover={{ y: -5, boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)' }} transition={{ type: 'spring', stiffness: 300 }}>
        <Card className="rounded-2xl overflow-hidden shadow-sm border border-slate-200/80">
            ...
            <CardContent className="p-4 text-sm text-slate-600 space-y-3">
                <p className="text-sm font-medium text-slate-700">You requested a partnership for their service with your product.</p>
        ...
        <Card className="rounded-2xl overflow-hidden shadow-sm border border-slate-200/80">
            ...
            {request.status === 'pending' && (
                <CardFooter className="flex justify-end gap-3 bg-slate-50/70 p-3 border-t">
                    <Button size="sm" variant="destructive" className="bg-red-500 hover:bg-red-600 text-white rounded-lg" onClick={() => handleRespond('declined')} disabled={isPending}>
                        <XCircle className="w-4 h-4 mr-2" />Decline
                    </Button>
                    <Button size="sm" className="bg-green-500 hover:bg-green-600 text-white rounded-lg" onClick={() => handleRespond('accepted')} disabled={isPending}>
...
const AcceptedPartnerCard = ({ partnership, currentUserId }: { partnership: PartnershipRequest, currentUserId?: string | null }) => {
  ...
  return (
    <motion.div whileHover={{ y: -5, boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)' }} transition={{ type: 'spring', stiffness: 300 }}>
        <Card className="rounded-2xl overflow-hidden shadow-sm border border-slate-200/80">
            <CardHeader className="flex flex-row items-start justify-between bg-teal-50/70 p-4 border-b border-teal-200">
                 <div className="flex items-center gap-4">
                    <Avatar className="h-14 w-14 border-4 border-white shadow-md">
                    ...
                    <div>
                        ...
                        <CardDescription className="text-xs font-medium text-teal-700">Active Partner</CardDescription>
                    </div>
                </div>
                <Badge className="capitalize flex items-center gap-1.5 text-xs py-1 px-3 rounded-full font-semibold border bg-teal-500 text-white border-teal-600">
...
const PartnershipTabContent = ({ requests, isLoading, isError, error, CardComponent, emptyState, currentUserId }: PartnershipTabContentProps) => {
  ...
  if (isError) {
    return <div className="p-8 text-red-500 text-center bg-red-50 rounded-2xl"><p>Error: {error?.message || 'An unknown error occurred.'}</p></div>;
  }
  ...
  return (
    ...
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 bg-slate-50/30 min-h-screen">
      <header className="mb-10">
        <h1 className="text-4xl font-extrabold tracking-tight text-slate-900">My Partnerships</h1>
        <p className="text-lg text-slate-600 mt-2">Manage your partnership requests and active collaborations.</p>
      </header>

      <Tabs defaultValue="sent" className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-slate-200/80 rounded-xl p-1.5 h-14">
          <TabsTrigger value="sent" className="text-base font-semibold data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-lg rounded-lg transition-all duration-300">Sent</TabsTrigger>
          <TabsTrigger value="received" className="text-base font-semibold data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-lg rounded-lg transition-all duration-300">Received</TabsTrigger>
          <TabsTrigger value="accepted" className="text-base font-semibold data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-lg rounded-lg transition-all duration-300">Partners</TabsTrigger>
        </TabsList>
