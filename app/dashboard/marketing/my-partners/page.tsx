            ...
            <CardHeader className="flex flex-row items-start justify-between bg-slate-100/80 p-5 border-b-2">
                <div className="flex items-center gap-4">
                    <Avatar className="h-16 w-16 border-4 border-white shadow-lg">
                        ...
                        <AvatarFallback className="text-xl font-bold">{request.serviceOwner.name.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div>
                        <CardTitle className="text-lg font-extrabold text-slate-800">{request.serviceOwner.name}</CardTitle>
                        <CardDescription className="text-sm font-medium text-slate-500">Request Sent</CardDescription>
                    </div>
                </div>
        ...
        <Card className="rounded-3xl overflow-hidden shadow-lg border-2 border-slate-200/80 h-full flex flex-col bg-white">
            <CardHeader className="flex flex-row items-start justify-between bg-slate-100/80 p-5 border-b-2">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <CardDescription className="text-sm font-medium text-slate-500">Request Received</CardDescription>
                    </div>
                </div>
                 <Badge className={cn('capitalize flex items-center gap-2 text-sm py-2 px-4 rounded-full font-bold border-2', statusUi.textColor, statusUi.bgColor, statusUi.borderColor)}>
                    {statusUi.icon}{statusUi.text}
            ...
            {request.status === 'pending' && (
                <CardFooter className="flex justify-end gap-4 bg-slate-100/80 p-4 border-t-2">
                    <Button size="lg" variant="destructive" className="bg-rose-500 hover:bg-rose-600 text-white rounded-xl font-bold text-base shadow-lg hover:shadow-xl transition-shadow" onClick={() => handleRespond('declined')} disabled={isPending}>
                        <XCircle className="w-5 h-5 mr-2" />Decline
                    </Button>
                    <Button size="lg" className="bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-bold text-base shadow-lg hover:shadow-xl transition-shadow" onClick={() => handleRespond('accepted')} disabled={isPending}>
...
const AcceptedPartnerCard = ({ partnership, currentUserId }: { partnership: PartnershipRequest, currentUserId?: string | null }) => {
  ...
  return (
    <motion.div
        ...
        className="h-full"
    >
        <Card className="rounded-3xl overflow-hidden shadow-lg border-2 border-cyan-200/80 h-full flex flex-col bg-white">
            <CardHeader className="flex flex-row items-start justify-between bg-cyan-50/80 p-5 border-b-2 border-cyan-200/80">
                 <div className="flex items-center gap-4">
                    <Avatar className="h-16 w-16 border-4 border-white shadow-lg">
                        <AvatarImage src={partner.profilePictureUrl || ''} alt={partner.name} />
                        <AvatarFallback className="text-xl font-bold">{partner.name.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div>
                        <CardTitle className="text-lg font-extrabold text-slate-800">{partner.name}</CardTitle>
                        <CardDescription className="text-sm font-bold text-cyan-700">Active Partner</CardDescription>
                    </div>
                </div>
                <Badge className="capitalize flex items-center gap-2 text-sm py-2 px-4 rounded-full font-bold border-2 bg-cyan-500 text-white border-cyan-600">
                    <Handshake className="h-5 w-5" />Partnership
                </Badge>
            </CardHeader>
            <CardContent className="p-5 text-base text-slate-700 space-y-4 flex-grow">
...
const PartnershipTabContent = ({ requests, isLoading, isError, error, CardComponent, emptyState, currentUserId }: PartnershipTabContentProps) => {
  ...
  if (isError) {
    return <div className="p-8 text-red-600 text-center bg-red-50 rounded-3xl border-2 border-dashed border-red-200"><p className="font-bold text-lg">Error: {error?.message || 'An unknown error occurred.'}</p></div>;
  }
  ...
  return (
    ...
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-slate-50 to-indigo-100/30 min-h-screen">
      <header className="mb-12 text-center">
        <h1 className="text-5xl font-black tracking-tighter text-slate-900">My Partnerships</h1>
        <p className="text-xl text-slate-600 mt-3 max-w-2xl mx-auto">Oversee your partnership requests and active collaborations with ease.</p>
      </header>

      <Tabs defaultValue="sent" className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-slate-200/90 rounded-2xl p-2 h-16 shadow-inner">
          <TabsTrigger value="sent" className="text-lg font-bold data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-xl rounded-xl transition-all duration-300">Sent</TabsTrigger>
          <TabsTrigger value="received" className="text-lg font-bold data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-xl rounded-xl transition-all duration-300">Received</TabsTrigger>
          <TabsTrigger value="accepted" className="text-lg font-bold data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-xl rounded-xl transition-all duration-300">Partners</TabsTrigger>
        </TabsList>
        ...
        <div className="mt-10">
            ...
            <PartnershipTabContent
                ...
                emptyState={<EmptyState icon={<Send size={64} />} title="No Sent Requests" description="You haven't sent any partnership requests yet. Start a new collaboration!" />}
            />
            </TabsContent>
            ...
            <PartnershipTabContent
                ...
                emptyState={<EmptyState icon={<GitPullRequest size={64} />} title="No Received Requests" description="You haven't received any new partnership requests yet. Check back soon!" />}
            />
            </TabsContent>
            ...
            <PartnershipTabContent
                ...
                emptyState={<EmptyState icon={<Users size={64} />} title="No Active Partners" description="You don't have any partners yet. Accepted requests will appear here." />}
